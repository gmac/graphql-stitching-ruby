...
describe 'GraphQL::Stitching::Composer, merging boundary queries' do
  def test_merges_boundaries_with_multiple_keys
    ...
    assert get_boundary(supergraph, "T", location: "a", selection: "upc", field: "a", arg: "upc")
    assert get_boundary(supergraph, "T", location: "b", selection: "upc", field: "b", arg: "upc")
    assert get_boundary(supergraph, "T", location: "b", selection: "id", field: "b", arg: "id")
    assert get_boundary(supergraph, "T", location: "c", selection: "id", field: "c", arg: "id")
  end
  ...
  def test_expands_interface_boundary_accessors_to_relevant_types
    ...
    assert get_boundary(supergraph, "Fruit", location: "a", selection: "id", field: "fruit", arg: "id")
    assert get_boundary(supergraph, "Apple", location: "a", selection: "id", field: "fruit", arg: "id")
    assert get_boundary(supergraph, "Banana", location: "a", selection: "id", field: "fruit", arg: "id")
    assert get_boundary(supergraph, "Apple", location: "b", selection: "id", field: "a", arg: "id")
    assert get_boundary(supergraph, "Banana", location: "b", selection: "id", field: "b", arg: "id")
  end
  ...
  def get_boundary(supergraph, type_name, location:, selection: nil, field: nil, arg: nil)
    supergraph.boundaries[type_name].find do |b|
      conditions = []
