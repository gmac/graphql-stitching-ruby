...
Each location that provides a unique variant of a type must provide at least one _resolver query_ for accessing the type by key. Type resolvers are root queries identified by a `@stitch` directive:

```graphql
directive @stitch(key: String!, arguments: String) repeatable on FIELD_DEFINITION
```

This directive (or [static configuration](#sdl-based-schemas)) marks a query used to fetch the type, and a `key` argument specifies a field needed from other locations to be used as a query argument.
...
Merged types must have a resolver query in each possible location. The one exception to this requirement are [outbound-only types](./docs/mechanics.md#outbound-only-merged-types) that contain no unique data, such as foreign keys:
