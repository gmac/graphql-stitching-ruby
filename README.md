...
Each location that provides a unique variant of a type must provide at least one _resolver query_ for accessing the type. Type resolvers are root queries identified by a `@stitch` directive:

```graphql
directive @stitch(key: String!, arguments: String) repeatable on FIELD_DEFINITION
```

This directive tells stitching how to cross-reference and fetch type variants from across locations, for example:
...
* The `@stitch` directive marks a root query where the merged type may be accessed. The merged type identity is inferred from the field return. This marker can also be provided as [static configuration](#sdl-based-schemas).
...
Merged types must have a resolver query in each of their possible locations. The one exception to this requirement are [outbound-only types](./docs/mechanics.md#outbound-only-merged-types) that contain no unique data, such as foreign keys:
...
The above type variant contains nothing but a key field that is available in other locations. Therefore, this variant will never require an inbound request to fetch it, and its resolver query may be omitted from this location.
