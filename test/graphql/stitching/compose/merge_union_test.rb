...
class GraphQL::Stitching::Compose::MergeUnionTest < Minitest::Test
  ...
  def test_merges_union_types
    ...
    assert_equal ["A", "B", "C"], schema.types["Thing"].possible_types.map(&:graphql_name).sort
  end
  ...
  def test_merges_union_descriptions
    ...
    assert_equal "a/b", schema.types["Thing"].description
  end
end
