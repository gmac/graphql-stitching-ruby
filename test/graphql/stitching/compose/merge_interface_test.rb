...
class GraphQL::Stitching::Compose::MergeInterfaceTest < Minitest::Test
  ...
  def test_merges_interface_memberships
    ...
    assert_equal ["A"], schema.types["AA"].interfaces.map(&:graphql_name).sort
    assert_equal ["B"], schema.types["BB"].interfaces.map(&:graphql_name).sort
    assert_equal ["A", "AA", "B", "BB"], schema.types["C"].interfaces.map(&:graphql_name).sort
  end
  ...
  def test_merges_interface_fields
    ...
    assert_equal ["code", "id", "name"], schema.types["I"].fields.keys.sort
    assert_equal ["code", "id", "name"], schema.types["T"].fields.keys.sort
  end
